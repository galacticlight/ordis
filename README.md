...
- **Memory:** phrases like "remember that I like tea", "note that I work nights", and "don't forget the foundry is loud" persist into OperatorMemory on disk (`memory.json` in user-data) and survive relaunch. "What do you remember" / "what do you know about me" reads back in-character, not as a raw dump unless asked.
- **Timers and reminders:** "timer 5 minutes", "set a timer for 90 seconds", "remind me in 10 minutes to stretch", "at 3pm", "in 20 minutes", "every morning remind me to stretch", "every weekday at 9am remind me to…". Pending jobs store `dueAt` + prompt (and optional `recurrence`) in user-data `tasks.json` (main process, not the renderer), reload when the app starts, and cap at twenty. Clock times and morning/weekday cadence use America/Los_Angeles. Ask "what's on the foundry", "what timers", or "list reminders" to hear pending jobs — recurring ones include the next fire. After a recurring fire, Ordis keeps the rule and schedules the next occurrence. Cancel with "cancel the timer" / "never mind the reminder", or name one: "cancel the stretch reminder".
- Mute / unmute steward TTS from Settings or by saying “mute yourself” / “unmute” (never clears Harbor or memory).
- Ask “what can you do” for a short steward inventory (foundry, memory, mute, Harbor status) without Harbor.

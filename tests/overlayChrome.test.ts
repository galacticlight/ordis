...
describe('Harbor vs steward cue', () => {
  it('Settings and overlay cue copy follow PublicSettings.hasApiKey', () => {
    ...
    expect(settings).toContain('clearApiKey: true')
    expect(settings).toContain('syncClearControl')
    expect(settingsHtml).toContain('id="key-status"')
    expect(settingsHtml).toContain('id="clearKey"')
    expect(settingsHtml).toMatch(/Unlink Harbor/)
    ...
    expect(main).toContain('clearApiKey')
    expect(main).toMatch(/patch\.clearApiKey === true/)
    expect(main).toMatch(/next\.apiKey = ''/)
    expect(main).toMatch(/else if \(typeof patch\.apiKey === 'string'\) next\.apiKey = patch\.apiKey\.trim\(\)/)
